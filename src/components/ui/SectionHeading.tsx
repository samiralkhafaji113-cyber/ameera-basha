  ...
  return (
    <div className={cn("flex flex-wrap items-end justify-between gap-x-6 gap-y-4", className)}>
      <div className="max-w-2xl">
        {eyebrow && (
          <p className={cn("mb-2 flex items-center gap-2 text-sm font-semibold", inverse ? "text-surface-sand-strong" : "text-accent-text")}>
            <span aria-hidden="true" className={cn("h-px w-8", inverse ? "bg-surface-sand-strong" : "bg-accent")} />
            {eyebrow}
          </p>
        )}
        <h2 id={id} className={inverse ? "!text-white" : undefined}>
          {title}
        </h2>
